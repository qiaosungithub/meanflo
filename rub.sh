...
HERE=/kmh-nfs-ssd-eu-mount/staging/sqa/sbsbsbsbsbsbssb/meanflow
sudo rsync -av . $HERE --exclude='.git'
sudo chmod 777 -R $HERE

gcloud compute tpus tpu-vm ssh $VM_NAME --zone $ZONE --command "ls /kmh-nfs-ssd-eu-mount && echo Starting at \$(date); source /etc/profile; cd $HERE; $CMD; echo Finished at \$(date)" --worker=all
